...
function buildHeadline(first, middle, final, triple) { return `${triple.theme}｜${first.name}→${middle.name}→${final.name}`; }
function summaryText(category, first, middle, final, triple, pair12, pair23, coverage) { const catName = CATEGORY_NAMES[category] || '综合'; return `本卦以【${first.name}】为起因、【${middle.name}】为经过、【${final.name}】为结果。问【${catName}】时，系统会调用完整规则层：${coverage.pairRules} 条二宫转化规则可覆盖全部 ${coverage.generatedTripleRules} 种三宫组合，并优先命中特殊三宫格局。当前格局为：${triple.meaning} 前段“${pair12.theme}”：${pair12.meaning} 后段“${pair23.theme}”：${pair23.meaning} 最终落在【${final.name}】，主调为：${final.result}`; }
function buildDetail(category, first, middle, final, pair12, pair23, triple, wuxing, advanced) {
  ...
  const values = [
    ...
    `${finalBase.points[1]} 最终【${final.name}】落局，转机多在${advanced.location.direction}、${advanced.location.places.join('、')}，或与${advanced.location.hint}有关的场景。${wuxing.processToResult.text}`,
    `${finalBase.points[2]} 三宫总势为“${triple.theme}”，行动上宜：${triple.advice}`,
    `节奏判断为${advanced.timing.pace}，参考窗口：${advanced.timing.window}。${advanced.timing.basis} 应期仅作传统文化参考。`
  ];
...
export function buildReading({ category, first, middle, final, shichenElement }) {
  ...
  return { category, categoryName: CATEGORY_NAMES[category] || '综合', score, verdict: category === 'general' ? verdictFrom(score, final) : final.categoryBase[category].verdict, headline: buildHeadline(first, middle, final, triple), keywords: [...new Set([...first.keywords.slice(0,2), ...middle.keywords.slice(0,2), ...final.keywords.slice(0,3), triple.theme, advanced.timing.pace])].slice(0,8), summary: summaryText(category, first, middle, final, triple, pair12, pair23, coverage), timeline: [{ label:'起因', palace:first.name, text:first.cause }, { label:'经过', palace:middle.name, text:middle.process }, { label:'结果', palace:final.name, text:final.result }], detail: category === 'general' ? [] : buildDetail(category, first, middle, final, pair12, pair23, triple, wuxing, advanced), combos: [triple, pair12, pair23], wuxing, advanced, advice: buildAdvice(category, first, middle, final, pair12, pair23, triple, advanced), coverage };
}
